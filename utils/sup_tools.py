# from Multvariate Time Series Transformer Framework
...
def compensate_masking(X, mask):
    ...
    num_active = torch.max(num_active, torch.ones(num_active.shape, 
...
def collate_superv_tslib(data, max_len=None):
    """Build mini-batch tensors from a list of (X, mask) tuples. Mask input. Create
    Args:
        data: len(batch_size) list of tuples (X, y).
            - X: torch tensor of shape (seq_length, feat_dim); variable seq_length.
    ...
    batch_size = len(data)
    features, labels, feature_mark_x, feature_mark_y = zip(*data)
